 ...
 * 
 * Requirements: 19.1, 19.2, 19.3, 19.5, 2.3
 */
...
export function SecuritySettings({ settings, onChange, isLoading = false }: SecuritySettingsProps) {
  ...
  const handleBiometricToggle = async () => {
    ...
    if (localSettings.biometricEnabled && biometricSetup) {
      // Disable biometric authentication
      try {
        await biometricService.removeBiometric();
        setBiometricSetup(false);
        handleChange('biometricEnabled', false);
      } catch (error) {
        console.error('Error removing biometric authentication:', error);
      }
    } else if (!localSettings.biometricEnabled && !biometricSetup) {
      // Enable biometric authentication - show setup
      setShowBiometricSetup(true);
    } else {
      // Just toggle the setting
      handleChange('biometricEnabled', !localSettings.biometricEnabled);
    }
  };
  ...
  const handleBiometricSetupComplete = (success: boolean) => {
    ...
    if (success) {
      ...
      handleChange('biometricEnabled', true);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <div className="flex-shrink-0">
                <div className={`flex h-10 w-10 items-center justify-center rounded-full ${
                  localSettings.biometricEnabled && biometricSetup ? 'bg-green-100 dark:bg-green-900/20' : 'bg-gray-100 dark:bg-gray-800'
                }`}>
                  <svg
                    className={`h-6 w-6 ${
                      localSettings.biometricEnabled && biometricSetup ? 'text-green-600 dark:text-green-400' : 'text-gray-400'
                    }`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
            ...
            <div className="flex items-center">
              <button
                ...
                aria-pressed={localSettings.biometricEnabled && biometricSetup}
                className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 ${
                  localSettings.biometricEnabled && biometricSetup ? 'bg-blue-600' : 'bg-gray-200 dark:bg-gray-700'
                }`}
              >
                <span
                  className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                    localSettings.biometricEnabled && biometricSetup ? 'translate-x-5' : 'translate-x-0'
                  }`}
                />
              </button>
            </div>
          ...
          {biometricSupported && !biometricSetup && !localSettings.biometricEnabled && (
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  <p className="text-sm text-blue-800 dark:text-blue-200">
                    ...
                    Click the toggle to set up biometric authentication.
                  </p>
                </div>
              </div>
            </div>
  ...
  // Show biometric setup modal if requested
  if (showBiometricSetup) {
    // We need user ID and master password for biometric setup
    // In a real implementation, you might need to prompt for master password
    // For now, we'll assume we have access to it or handle it differently
    return (
      <>
        ...
        <div className="space-y-6">
          ...
          {/* Settings content would be here but hidden by modal */}
        </div>
        
        {/* Biometric Setup Modal */}
        <BiometricSetup
          userId="current-user" // This should come from auth context
          masterKey="" // This would need to be provided securely
